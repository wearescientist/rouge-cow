...
﻿class MapGenerator {
    ...
    buildFixedFloor7Rooms() {
        ...
        awakening.doors.up = { open: false, locked: true, target: boss };
...
const SURVIVOR_CONFIG = {
    ...
    getCombatAreaBounds(room, innerRatio = 0.9) {
        const safeRatio = Math.max(0.5, Math.min(0.98, innerRatio || 0.9));
        const floorLeft = this.floorLeft;
        const floorTop = this.floorTop;
        const floorRight = this.floorRight;
        const floorBottom = this.floorBottom;
        const width = Math.max(1, floorRight - floorLeft);
        const height = Math.max(1, floorBottom - floorTop);
        const insetX = (width * (1 - safeRatio)) * 0.5;
        const insetY = (height * (1 - safeRatio)) * 0.5;

        return {
            minX: floorLeft + insetX,
            maxX: floorRight - insetX,
            minY: floorTop + insetY,
            maxY: floorBottom - insetY
        };
    },

    clampEnemyToCombatArea(x, y, room, innerRatio = 0.9) {
        const bounds = this.getCombatAreaBounds(room, innerRatio);
        return {
