(function(global){
  ...
  class PerformanceProfiler {
    ...
    getDisplayFrameStats(){
      const frameIntervals = [];
      for (let i = 1; i < this.frameHistory.length; i += 1) {
        const prev = this.frameHistory[i - 1];
        const current = this.frameHistory[i];
        if (!prev || !current) continue;
        const delta = current.stamp - prev.stamp;
        if (Number.isFinite(delta) && delta > 0) frameIntervals.push(delta);
      }
      if (frameIntervals.length > 0) {
        const avgFrame = frameIntervals.reduce((a,b)=>a+b,0) / frameIntervals.length;
        return {
          avgFrame,
          p95Frame: this.percentile(frameIntervals, 0.95),
          worstFrame: Math.max(...frameIntervals),
          fps: avgFrame > 0 ? 1000 / avgFrame : 0
        };
      }
      const perfSummary = this.game?.perfMonitor?.getSummary?.() || null;
      const fallbackFrame = Number(perfSummary?.frameTime || 0);
      const fallbackFps = Number(perfSummary?.avgFps || perfSummary?.fps || 0);
      return {
        avgFrame: fallbackFrame,
        p95Frame: fallbackFrame,
        worstFrame: Number(perfSummary?.worstFrameTime || fallbackFrame),
        fps: fallbackFps
      };
    }
    ...
    getSnapshot(limit = 16){
      this.ensureBound();
      const cpuFrameTotals = this.frameHistory.map(f => f.total);
      const cpuAvgFrame = cpuFrameTotals.length ? cpuFrameTotals.reduce((a,b)=>a+b,0) / cpuFrameTotals.length : 0;
      const cpuWorstFrame = cpuFrameTotals.length ? Math.max(...cpuFrameTotals) : 0;
      const cpuP95Frame = this.percentile(cpuFrameTotals, 0.95);
      const cpuFps = cpuAvgFrame > 0 ? 1000 / cpuAvgFrame : 0;
      const displayStats = this.getDisplayFrameStats();
      const sections = this.buildSectionRows(cpuAvgFrame);
      return {
        enabled: this.enabled,
        frames: this.frameHistory.length,
        avgFrame: displayStats.avgFrame,
        p95Frame: displayStats.p95Frame,
        worstFrame: displayStats.worstFrame,
