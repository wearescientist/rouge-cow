(function attachHiddenRoomSystem(global) {
    ...
    const HIDDEN_ROOM_WARMED_FLOORS = new Set();


    const hiddenProgressApi = global.HiddenRoomProgressRuntime.createApi({
        global,
        STORAGE_KEY,
        HIDDEN_ROOM_VERSION,
        HIDDEN_ROOM_PROFILES,
    ...
    function cameraShake(game, intensity, duration) {
        ...
        if (typeof camera.addShake === 'function') {
            camera.addShake(intensity, duration || 0.35);
            return;
        }
    ...
    function resolveSecretroomSpritePath(pathOrFile = '') {
        const normalized = String(pathOrFile || '')
            .replace(/^\.\//, '')
            .replace(/^\//, '')
            .replace(/^assets\/runtime\/sprites\//, '');
        const secretPath = normalized.startsWith('secretroom/')
            ? normalized
            : `secretroom/${normalized.replace(/^secretroom\//, '')}`;
        return global.RuntimeAssetBase?.resolveSprite?.(secretPath) || `${SECRETROOM_ASSET_BASE}${secretPath.replace(/^secretroom\//, '')}`;
    }

    function warmHiddenRoomAssets(floor) {
        const floorKey = Number.isFinite(floor) ? floor : 0;
        if (HIDDEN_ROOM_WARMED_FLOORS.has(floorKey)) return 0;
        HIDDEN_ROOM_WARMED_FLOORS.add(floorKey);
        let warmed = 0;
        Object.entries(SECRETROOM_FILES).forEach(([key, file]) => {
            if (SECRETROOM_SPRITES[key]) return;
            const img = new Image();
            img.decoding = 'async';
            img.loading = 'eager';
            img.src = resolveSecretroomSpritePath(file);
    ...
    function getGlobalEntityBrightness() {
        const raw = Number(global.game?.runtimeSettings?.entityBrightness ?? 0.4);
        return Number.isFinite(raw) ? Math.max(0, Math.min(1, raw)) : 0.4;
    }
    ...
    function drawSpriteCentered(ctx, camera, key, x, y, drawW, drawH, options = {}) {
        ...
        const entityBrightness = options.entityBrightness === false ? 1 : getGlobalEntityBrightness();
        const brightness = (Number.isFinite(options.brightness) ? options.brightness : 1) * entityBrightness;
    ...
    function drawImageCentered(ctx, camera, image, x, y, drawW, drawH, options = {}) {
        ...
        if (entityBrightness !== 1) ctx.filter = `brightness(${entityBrightness})`;
        const halfW = renderW * 0.5;
        const halfH = renderH * 0.5;
    ...
    function playCrystalVoiceTick(kind) {
        try {
            ...
            playCrystalVoiceTick._ctx = playCrystalVoiceTick._ctx || new AudioContext();
            const ctx = playCrystalVoiceTick._ctx;
    ...
    function setupRoom(game, room) {
        ...
        ensureHiddenRoomTotemPickup(game, room, floor);
        saveState(game);
        return room.hiddenMode;
    }
    ...
    function drawCandleNode(ctx, camera, node, lit, time) {
        ...
        drawSpriteCentered(ctx, camera, 'dec_candle', node.x, node.y, 58 * nodeScale, 58 * nodeScale, {
            ...
            exactSize: true,
            brightness: lit ? (1 + 0.12 * lanternMul) : 1
        });
        ...
        const seed = (Number.isFinite(node.index) ? node.index : 0) * 1.731 + (node.x || 0) * 0.013 + (node.y || 0) * 0.009;
        ctx.save();
        if (lit) {
            const flickerA = Math.sin(time * 4.8 + seed * 1.17);
            const flickerB = Math.sin(time * 7.6 + seed * 2.41);
            const flicker = flickerA * 0.08 + flickerB * 0.045;
            const pulse = 1 + flicker;
            const sway = (Math.sin(time * 2.15 + seed * 0.93) * 0.42 + Math.sin(time * 3.75 + seed * 1.61) * 0.26) * nodeScale;
            const coreX = flameX + sway;
            const coreY = flameY - 1.2 * nodeScale + Math.sin(time * 3.05 + seed * 1.24) * 0.4 * nodeScale;
            const warmMul = 0.58 + lanternMul * 0.42;

            ctx.globalCompositeOperation = 'screen';
            const halo = ctx.createRadialGradient(coreX, coreY - 2.8 * nodeScale, 0, coreX, coreY - 2.8 * nodeScale, 20 * nodeScale);
            halo.addColorStop(0, `rgba(255,198,126,${0.22 * warmMul})`);
            halo.addColorStop(0.38, `rgba(255,154,84,${0.11 * warmMul})`);
            halo.addColorStop(1, 'rgba(255,132,70,0)');
            ctx.fillStyle = halo;
            ctx.beginPath();
            ctx.arc(coreX, coreY - 2.8 * nodeScale, 20 * nodeScale, 0, Math.PI * 2);
            ctx.fill();

            const ember = ctx.createRadialGradient(coreX, coreY - 0.8 * nodeScale, 0, coreX, coreY - 0.8 * nodeScale, 10 * nodeScale);
            ember.addColorStop(0, `rgba(255,216,152,${0.26 * warmMul})`);
            ember.addColorStop(0.46, `rgba(255,166,90,${0.12 * warmMul})`);
            ember.addColorStop(1, 'rgba(255,140,72,0)');
            ctx.fillStyle = ember;
            ctx.beginPath();
            ctx.arc(coreX, coreY - 0.8 * nodeScale, 10 * nodeScale, 0, Math.PI * 2);
            ctx.fill();

            ctx.globalCompositeOperation = 'source-over';
            ctx.fillStyle = 'rgba(255,170,92,0.88)';
            ctx.beginPath();
            ctx.ellipse(coreX, coreY + 0.7 * nodeScale, 2.55 * nodeScale, 5.4 * nodeScale * pulse, 0, 0, Math.PI * 2);
            ctx.fill();

            ctx.fillStyle = 'rgba(255,204,124,0.78)';
            ctx.beginPath();
            ctx.ellipse(coreX, coreY - 1.25 * nodeScale, 1.7 * nodeScale, 3.65 * nodeScale * pulse, 0, 0, Math.PI * 2);
            ctx.fill();

            ctx.fillStyle = 'rgba(255,235,186,0.50)';
            ctx.beginPath();
            ctx.ellipse(coreX, coreY - 2.15 * nodeScale, 0.75 * nodeScale, 1.65 * nodeScale * pulse, 0, 0, Math.PI * 2);
            ctx.fill();
        } else {
    ...
    function drawDormantMushroomNode(ctx, camera, node, sprite, options = {}) {
        drawGlowingMushroomSprite(ctx, camera, sprite.key, node.x, node.y, sprite.drawW, sprite.drawH, {
            offsetY: sprite.offsetY,
            alpha: Number.isFinite(options.alpha) ? options.alpha : 0.84,
            brightness: Number.isFinite(options.brightness) ? options.brightness : 0.84,
            glowColor: sprite.glowColor,
            rotation: node.rotation,
            haloRadius: Number.isFinite(options.haloRadius) ? options.haloRadius : 22,
            floorRadius: Number.isFinite(options.floorRadius) ? options.floorRadius : 18,
            floorW: Number.isFinite(options.floorW) ? options.floorW : 12,
            floorH: Number.isFinite(options.floorH) ? options.floorH : 6,
            haloAlpha0: Number.isFinite(options.haloAlpha0) ? options.haloAlpha0 : 0.12,
            haloAlpha1: Number.isFinite(options.haloAlpha1) ? options.haloAlpha1 : 0.04,
            floorAlpha0: Number.isFinite(options.floorAlpha0) ? options.floorAlpha0 : 0.10,
            floorAlpha1: Number.isFinite(options.floorAlpha1) ? options.floorAlpha1 : 0.04,
            spriteGlowAlpha: Number.isFinite(options.spriteGlowAlpha) ? options.spriteGlowAlpha : 0.16,
            glowBlur: Number.isFinite(options.glowBlur) ? options.glowBlur : 10,
            pulse: 1
        });
    }
    ...
    function drawMushroomNode(ctx, camera, node, state, time, completed = false) {
        ...
        if (!mode) {
            drawDormantMushroomNode(ctx, camera, node, sprite, { alpha: 0.82, brightness: 0.78 });
            return;
        }
    ...
    function drawMemoryMushroomNode(ctx, camera, node, progress, time) {
        ...
        if (!solvedLit) {
            drawDormantMushroomNode(ctx, camera, node, sprite, { alpha: 0.74, brightness: 0.68 });
            return;
        }
...
function drawHiddenLayoutEditorOverlay(ctx, camera, room, game) {
        ...
        if (!editor.active || !isHiddenLayoutEditableRoom(room)) return;
        ...
        if (selected) {
            ...
            const alpha = Number.isFinite(selected.obj.alpha) ? selected.obj.alpha : 1;
            const label = `${selected.label}  缩放 ${scale.toFixed(2)}  透明 ${alpha.toFixed(2)}  旋转 ${rotationDeg}°  图层 ${(selected.obj.editorLayer || 0)}`;
    ...
    const hiddenLayoutRuntimeState = {
        editor: {
            active: false,
            selectedId: null,
            hoverId: null,
    ...
    function getHiddenLayoutRuntimeState() {
        return hiddenLayoutRuntimeState;
    }

    function ensureBuiltinHiddenLayoutAssets() {
        if (hiddenLayoutRuntimeState.assets.length > 0) return hiddenLayoutRuntimeState.assets;
        hiddenLayoutRuntimeState.assets = BUILTIN_HIDDEN_LAYOUT_ASSET_SOURCES.map((src, index) => {
            const normalizedSrc = global.RuntimeAssetBase?.resolve?.(src) || src;
            const fileName = String(src || '').split('/').pop() || `hidden_asset_${index + 1}.png`;
            return {
        ...
        return hiddenLayoutRuntimeState.assets;
    }

    function getHiddenLayoutEditorState() {
        return hiddenLayoutRuntimeState.editor;
    }

    function setHiddenLayoutLastAction(action) {
        hiddenLayoutRuntimeState.lastAction = action || null;
        return hiddenLayoutRuntimeState.lastAction;
    }

    function getBaseHiddenLayoutStore() {
        const base = clone(global.HIDDEN_ROOM_LAYOUT_OVERRIDES || { version: HIDDEN_ROOM_VERSION, floors: {}, sharedTargets: {} }) || {};
        if (!base.floors || typeof base.floors !== 'object') base.floors = {};
        if (!base.sharedTargets || typeof base.sharedTargets !== 'object') base.sharedTargets = {};
        return base;
    }

    function mergeHiddenLayoutStores(baseStore, draftStore) {
        const merged = clone(baseStore || { version: HIDDEN_ROOM_VERSION, floors: {}, sharedTargets: {} }) || {};
        if (!merged.floors || typeof merged.floors !== 'object') merged.floors = {};
        if (!merged.sharedTargets || typeof merged.sharedTargets !== 'object') merged.sharedTargets = {};
        if (!draftStore || typeof draftStore !== 'object') return merged;
        if (draftStore.sharedTargets && typeof draftStore.sharedTargets === 'object') {
            Object.entries(draftStore.sharedTargets).forEach(([key, value]) => {
                merged.sharedTargets[key] = clone(value);
            });
        }
        if (draftStore.floors && typeof draftStore.floors === 'object') {
            Object.entries(draftStore.floors).forEach(([floorKey, floorValue]) => {
                if (!merged.floors[floorKey] || typeof merged.floors[floorKey] !== 'object') merged.floors[floorKey] = {};
                Object.entries(floorValue || {}).forEach(([targetKey, targetValue]) => {
                    merged.floors[floorKey][targetKey] = clone(targetValue);
                });
            });
        }
        return merged;
    ...
    function getHiddenLayoutStore() {
        if (hiddenLayoutRuntimeState.store) return hiddenLayoutRuntimeState.store;
        const baseStore = getBaseHiddenLayoutStore();
        let parsed = null;
        try {
            const raw = localStorage.getItem(HIDDEN_LAYOUT_STORAGE_KEY);
            parsed = raw ? JSON.parse(raw) : null;
        } catch (err) {
            console.warn('[HiddenRoomLayoutEditor] load store failed', err);
        }
        hiddenLayoutRuntimeState.store = mergeHiddenLayoutStores(baseStore, parsed);
        return hiddenLayoutRuntimeState.store;
    }

    function saveHiddenLayoutStore() {
        const store = getHiddenLayoutStore();
        hiddenLayoutRuntimeState.store = store;
        try {
            localStorage.setItem(HIDDEN_LAYOUT_STORAGE_KEY, JSON.stringify(store));
        } catch (err) {
            console.warn('[HiddenRoomLayoutEditor] save store failed', err);
        }
        return store;
    }
    ...
    function getFloorLayoutOverrides(floor) {
        const store = getHiddenLayoutStore();
        const key = String(Number.isFinite(floor) ? floor : 0);
        if (!store.floors[key] || typeof store.floors[key] !== 'object') {
            store.floors[key] = {};
        }
        return store.floors[key];
    }

    function getResolvedFloorLayoutOverrides(floor) {
        const store = getHiddenLayoutStore();
        const resolved = clone(store.sharedTargets || {});
        const floorStore = store.floors?.[String(Number.isFinite(floor) ? floor : 0)] || {};
        Object.entries(floorStore).forEach(([key, value]) => {
    ...
    function clearHiddenLayoutOverridesForFloor(floor) {
        const store = getHiddenLayoutStore();
        delete store.floors[String(Number.isFinite(floor) ? floor : 0)];
        saveHiddenLayoutStore();
    }

    function getHiddenLayoutAssetLibrary() {
        return ensureBuiltinHiddenLayoutAssets();
    }

    function installHiddenLayoutEditorInput() {
        if (hiddenLayoutRuntimeState.inputInstalled) return;
        hiddenLayoutRuntimeState.inputInstalled = true;
        global.addEventListener?.('keydown', (event) => {
            if (event.key === 'Escape') {
                getHiddenLayoutEditorState().dragging = null;
            }
        });
    }

    async function importHiddenLayoutAssets(fileList) {
        const files = Array.from(fileList || []).filter((file) => file && /^image\//.test(file.type || ''));
        const loaded = files.map((file, index) => ({
            id: `user_asset_${Date.now()}_${index}`,
            name: file.name || `素材_${index + 1}`,
            src: URL.createObjectURL(file),
            width: 128,
            height: 128,
            kind: 'custom_image'
        }));
        hiddenLayoutRuntimeState.assets = hiddenLayoutRuntimeState.assets.concat(loaded);
        return loaded;
    }

    function isHiddenLayoutEditableRoom(room) {
        return !!room && (room.type === 'hidden' || (Number(room.floor) === 7 && room.floor7Role === 'awakening'));
    }

    function getActiveHiddenLayoutRoom(game) {
        return isHiddenLayoutEditableRoom(game?.curRoom) ? game.curRoom : null;
    }

    function collectHiddenLayoutTargets(room, floor) {
        if (!room) return [];
        if (Number(room.floor) === 7 && room.floor7Role === 'awakening') {
            return global.game?.floor7AwakeningSystem?.getLayoutEditorTargets?.(room) || [];
        }
        const targets = [];
        (room.hiddenFloor1Panels || []).forEach((item) => {
            if (!item || !item.layoutId) return;
            targets.push({ id: item.layoutId, kind: item.kind || 'panel', label: item.layoutLabel || item.layoutId, obj: item });
        });
        (room.hiddenTemplateCells || []).forEach((item) => {
            if (!item || !item.layoutId) return;
            targets.push({ id: item.layoutId, kind: item.kind || 'template', label: item.layoutLabel || item.layoutId, obj: item });
        });
        (room.hiddenPuzzleNodes || []).forEach((item) => {
            if (!item || !item.layoutId) return;
            targets.push({ id: item.layoutId, kind: item.kind || 'node', label: item.layoutLabel || labelForNodeKind(item), obj: item });
        });
        [room.hiddenOrbPedestal, room.hiddenOrb, room.hiddenLegacyTable, room.hiddenLegacyCandle].forEach((item) => {
            if (!item || !item.layoutId) return;
            targets.push({ id: item.layoutId, kind: item.kind || 'node', label: item.layoutLabel || labelForNodeKind(item), obj: item });
        });
        (room.hiddenDecor || []).forEach((item, index) => {
            if (!item || !item.layoutId) return;
            targets.push({
                id: item.layoutId,
                kind: item.kind || 'decor',
                label: item.layoutLabel || item.customAssetName || `装饰 ${index + 1}`,
                obj: item
            });
        });
    ...
    function getSelectedHiddenLayoutTarget(room) {
        ...
        const selected = targets.find((item) => item.id === editor.selectedId) || targets[0];
        if (!editor.selectedId) editor.selectedId = selected.id;
        return selected;
    }

    function moveHiddenLayoutSelection(room, step) {
        const editor = getHiddenLayoutEditorState();
        const targets = collectHiddenLayoutTargets(room, room?.hiddenRoomFloor || 0);
        if (!targets.length) {
        ...
        const currentIndex = Math.max(0, targets.findIndex((item) => item.id === editor.selectedId));
        const nextIndex = (currentIndex + (step || 0) + targets.length) % targets.length;
        editor.selectedId = targets[nextIndex].id;
        return targets[nextIndex];
    }

    function getLayoutTargetScreenBounds(camera, target) {
        const obj = target?.obj || {};
        const pos = camera?.worldToScreen ? camera.worldToScreen(obj.x || 0, obj.y || 0) : { x: obj.x || 0, y: obj.y || 0 };
        const w = Math.max(18, Number(obj.w || obj.drawW || obj.radius * 2 || 48));
        const h = Math.max(18, Number(obj.h || obj.drawH || obj.radius * 2 || 48));
        return {
            x: pos.x - w * 0.5,
            y: pos.y - h * 0.5,
            w,
    ...
    function labelForNodeKind(node) {
        ...
        return labels[node?.kind] || (node?.kind ? String(node.kind) : '布局节点');
    }

    function registerLayoutTarget(obj, layoutId, layoutLabel) {
        if (!obj || typeof obj !== 'object') return obj;
        obj.layoutId = layoutId || obj.layoutId || `layout_${Date.now()}_${Math.random().toString(36).slice(2, 7)}`;
        obj.layoutLabel = layoutLabel || obj.layoutLabel || labelForNodeKind(obj);
        return obj;
    }

    function getSortedEditorList(list) {
        return (Array.isArray(list) ? list.slice() : []).sort((a, b) => {
            const layerA = Number.isFinite(a?.editorLayer) ? a.editorLayer : 0;
            const layerB = Number.isFinite(b?.editorLayer) ? b.editorLayer : 0;
            if (layerA !== layerB) return layerA - layerB;
            const yA = Number.isFinite(a?.y) ? a.y : 0;
            const yB = Number.isFinite(b?.y) ? b.y : 0;
            if (yA !== yB) return yA - yB;
            return String(a?.layoutId || a?.id || '').localeCompare(String(b?.layoutId || b?.id || ''));
        });
    }

    function ensureLayoutTargetDefaults(obj) {
        if (!obj || typeof obj !== 'object') return obj;
        if (!Number.isFinite(obj.scale)) obj.scale = 1;
        if (!Number.isFinite(obj.scaleX)) obj.scaleX = 1;
        if (!Number.isFinite(obj.scaleY)) obj.scaleY = 1;
        if (!Number.isFinite(obj.rotation)) obj.rotation = 0;
        if (!Number.isFinite(obj.alpha)) obj.alpha = 1;
        if (!Number.isFinite(obj.editorLayer)) obj.editorLayer = 0;
        return obj;
    }

    function nudgeHiddenLayoutTarget(target, dx, dy) {
        if (!target?.obj) return;
        target.obj.x = Number(target.obj.x || 0) + Number(dx || 0);
        target.obj.y = Number(target.obj.y || 0) + Number(dy || 0);
    }

    function resizeHiddenLayoutTargetObject(obj, dw, dh) {
        if (!obj) return false;
        if (Number.isFinite(obj.w) || Number.isFinite(obj.h)) {
            obj.w = Math.max(8, Number(obj.w || obj.drawW || 48) + Number(dw || 0));
            obj.h = Math.max(8, Number(obj.h || obj.drawH || 48) + Number(dh || 0));
            return true;
        }
        if (Number.isFinite(obj.drawW) || Number.isFinite(obj.drawH)) {
            obj.drawW = Math.max(8, Number(obj.drawW || obj.w || 48) + Number(dw || 0));
            obj.drawH = Math.max(8, Number(obj.drawH || obj.h || 48) + Number(dh || 0));
            return true;
        }
        if (Number.isFinite(obj.radius)) {
            obj.radius = Math.max(4, Number(obj.radius || 24) + Number(dw || dh || 0) * 0.5);
            return true;
        }
        return false;
    }

    function createHiddenDecorInstanceFromAsset(asset, floor, room, cameraAnchor) {
        if (!asset) return null;
        hiddenLayoutRuntimeState.decorCounter += 1;
        return {
            layoutId: `custom_decor_${floor}_${hiddenLayoutRuntimeState.decorCounter}`,
            layoutLabel: asset.name || '自定义装饰',
            kind: 'custom_decor',
            customAssetName: asset.name || '自定义装饰',
            imageSrc: asset.src,
            x: Number(cameraAnchor?.x) || roomCenterX(room),
            y: Number(cameraAnchor?.y) || roomCenterY(room),
            drawW: Number(asset.width) || 128,
            drawH: Number(asset.height) || 128,
            alpha: 1,
            scale: 1,
            rotation: 0,
            editorLayer: 0
    ...
    function removeHiddenDecorInstance(floor, layoutId) {
        ...
        delete floorStore[String(layoutId)];
        saveHiddenLayoutStore();
    }
    ...
    function getHiddenLayoutFloorCustomDecor(floor) {
        const floorStore = getResolvedFloorLayoutOverrides(floor);
        return Object.values(floorStore || {})
            .filter((item) => item && typeof item === 'object' && /^custom_decor_/.test(String(item.layoutId || '')))
            .map((item) => clone(item));
    }
    ...
    function captureHiddenLayoutOverrides(room, floor) {
        ...
        collectHiddenLayoutTargets(room, floor).forEach((target) => {
            if (!target?.id || !target?.obj) return;
            floorStore[target.id] = clone(target.obj);
        });
        saveHiddenLayoutStore();
        return floorStore;
    }
    ...
    function applyHiddenLayoutOverrides(room, floor) {
        const floorStore = getResolvedFloorLayoutOverrides(floor);
        const targetMap = new Map(collectHiddenLayoutTargets(room, floor).map((target) => [target.id, target.obj]));
        Object.entries(floorStore || {}).forEach(([targetId, override]) => {
            const target = targetMap.get(targetId);
            if (!target || !override || typeof override !== 'object') return;
            Object.entries(override).forEach(([key, value]) => {
                if (key === 'layoutId' || key === 'layoutLabel' || key === 'kind') return;
    ...
    function getHiddenLayoutEditorSelectionInfo(game) {
        ...
        const target = room ? getSelectedHiddenLayoutTarget(room) : null;
    ...
    function seededPermutation(size, seed) {
        ...
        return shuffleWithRng(Array.from({ length: count }, (_, index) => index), seededRng(seed));
    }
    ...
    function drawHiddenMushroomBand(ctx, camera, item, decorMode, tuning) {
        ...
        const entityBrightness = getGlobalEntityBrightness();
        const brightnessBase = (active ? 1.92 : (preview ? 1.58 : 1.04)) * entityBrightness;
    ...
    function buildPuzzleNodes(room, floor, progress) {
        ...
        if (Array.isArray(room.hiddenFloor1Panels)) {
            room.hiddenFloor1Panels.forEach((panel, index) => {
                ...
                registerLayoutTarget(panel, `floor1_panel_${side}`, `面板${side === 'left' ? '左' : '右'}`);
            });
        }
        if (Array.isArray(room.hiddenTemplateCells)) {
            room.hiddenTemplateCells.forEach((cell, index) => registerLayoutTarget(cell, `template_${index}`, `模板蘑菇${index + 1}`));
    ...
    let bottomCaptionSequenceToken = 0;

    function closeBottomCaption(reason = 'close') {
    ...
    function ensureBottomCaptionInput() {
        ...
        document.addEventListener('keydown', (event) => {
            ...
            if (event.key === 'Escape') {
                event.preventDefault();
                event.stopPropagation();
                closeBottomCaption('cancel');
            }
        }, true);
    }

    function showBottomCaption(options) {
        closeBottomCaption('replace');
        ...
        const baseSpeed = Number.isFinite(options?.speed) ? options.speed : 54;
        const visibleChars = Math.max(1, fullText.replace(/\s/g, '').length);
        const speed = Math.max(28, Math.min(baseSpeed, Math.ceil(1500 / visibleChars)));
        ...
        const close = (reason = 'close') => closeBottomCaption(reason);
        state.autoTimer = setTimeout(close, Math.max(1700, fullText.length * Math.max(10, speed + 6) + 260));
        card.addEventListener('click', () => {
    ...
    function showBottomCaptionSequence(lines, options = {}) {
        const queue = (Array.isArray(lines) ? lines : [])
            .map(line => String(line || '').trim())
            .filter(Boolean);
        if (queue.length <= 0) {
            showBottomCaption(options);
            return;
        }

        const sequenceToken = ++bottomCaptionSequenceToken;
        const playNext = (index) => {
            if (sequenceToken !== bottomCaptionSequenceToken) return;
            ...
            showBottomCaption({
                ...
                onClose: (reason) => {
                    if (sequenceToken !== bottomCaptionSequenceToken) return;
                    if (reason === 'cancel' || reason === 'replace') return;
    ...
    function showNote(profile, extraHtml) {
        const storyLines = Array.isArray(profile?.lines) && profile.lines.length > 0
            ? profile.lines
            : (global.StoryDialogueData?.splitIntoSentences?.(profile?.note || '') || []);
        showBottomCaptionSequence(storyLines, {
    ...
    function getHiddenTotemSpawnPoint(room, floor) {
        if (!room) return null;
        if (floor === 6) {
            const diaryNode = (room.hiddenPuzzleNodes || []).find(node => node?.kind === 'legacy_book');
            if (diaryNode) {
                return { x: diaryNode.x, y: diaryNode.y + 118 };
            }
        }
        if (room.hiddenOrb) {
            return { x: room.hiddenOrb.x, y: room.hiddenOrb.y + 126 };
        }
        return null;
    }

    function ensureHiddenRoomTotemPickup(game, room, floor) {
        const progress = getFloorProgress(game, floor);
        const totemId = HIDDEN_TOTEM_ID_BY_FLOOR[floor];
        if (!room || !totemId || progress.totemCollected || !progress.witnessed) return false;
        room.items = Array.isArray(room.items) ? room.items : [];
        const exists = room.items.some(item => item?.type === 'totem' && item?.hiddenTotemFloor === floor);
        if (exists) return false;

        const spawn = getHiddenTotemSpawnPoint(room, floor);
        if (!spawn) return false;

        const totem = (typeof TOTEMS !== 'undefined' && TOTEMS[totemId]) ? TOTEMS[totemId] : null;
        room.items.push({
            x: spawn.x,
            y: spawn.y,
            type: 'totem',
            totemId,
            hiddenTotemFloor: floor,
            icon: totem?.icon || '🗿',
            name: totem?.name || `图腾 ${totemId}`
        });
        game?.particles?.sparkle?.(spawn.x, spawn.y - 10, '#ffe27a', 14);
        game?.damageNumbers?.spawn?.(spawn.x, spawn.y - 28, '图腾出现', {
            color: '#ffe27a',
            size: 16,
            life: 1.6
        });
    ...
    function handleNodeInteract(game, room, node) {
        ...
        if (floor === 6 && node.kind === 'legacy_book') {
            ...
            ensureHiddenRoomTotemPickup(game, room, 6);
    ...
    function drawFloor1TemplateBoard(ctx, camera, room, time) {
        ...
        // floor1 panels intentionally removed; keep only subtle stage glow and the actual puzzle pieces.
        ctx.restore();
    ...
    function patchRoomDraw() {
        return global.HiddenRoomRenderRuntime.patchRoomDraw(Room, {
            global,
            drawHiddenWorldLayer
        });
    }
    ...
    function patchGamePrototype() {
        ...
        global.HiddenRoomRenderRuntime.patchGameDraw(Game, {
            global,
            getFloor,
            getFloorProgress,
            drawVignette,
            drawFloor4Overlay,
            drawHiddenLayoutEditorOverlay,
            drawHiddenInteractCue,
            drawBanner
        });
        ...
        global.HiddenRoomEditorRuntime.patchGamePrototype(Game, {
            global,
            clamp,
            getFloor,
            roomCenterX,
            ...
            getHiddenLayoutEditorState,
            getSelectedHiddenLayoutTarget,
            getActiveHiddenLayoutRoom,
            captureHiddenLayoutOverrides,
            clearHiddenLayoutOverridesForFloor,
            moveHiddenLayoutSelection,
            nudgeHiddenLayoutTarget,
            resizeHiddenLayoutTargetObject,
            importHiddenLayoutAssets,
            getHiddenLayoutAssetLibrary,
            setHiddenLayoutLastAction,
            createHiddenDecorInstanceFromAsset,
            removeHiddenDecorInstance,
            clearHiddenDecorInstancesForFloor,
            getFloorLayoutOverrides,
            saveHiddenLayoutStore,
            getHiddenLayoutEditorSelectionInfo,
            getHiddenLayoutRuntimeState,
            getHiddenLayoutStore,
    ...
    function ensureHiddenLayoutEditorGameMethods() {
        return global.HiddenRoomEditorRuntime.ensureGameMethods(Game, {
            global,
            clamp,
            getFloor,
            roomCenterX,
    ...
    function bootstrap() {
        ...
        global.HiddenRoomSystemRuntime = {
            ...
            createDefaultState,
            prewarmAssets(floor) {
                return warmHiddenRoomAssets(floor);
            },
            drawCandleNode(ctx, camera, node, lit, time) {
                drawCandleNode(ctx, camera, node, lit, time);
            },
            drawSecretSpriteCentered(ctx, camera, key, x, y, drawW, drawH, options) {
                return drawSpriteCentered(ctx, camera, key, x, y, drawW, drawH, options || {});
            },
            applyHiddenLayoutOverrides(room, floor) {
                return applyHiddenLayoutOverrides(room, floor);
            },
            markTotemCollected(game, floor) {
                if (!game || !floor) return false;
                const state = ensureState(game);
                const progress = ensureFloorState(state, floor);
                if (progress.totemCollected) return false;
                progress.totemCollected = true;
                saveState(game);
                return true;
            },
            drawFloor2Critters,
            getLayoutEditorState: getHiddenLayoutEditorState,
            getLayoutOverridesForFloor(floor) {
                return clone(getResolvedFloorLayoutOverrides(floor));
            },
            captureHiddenLayoutOverrides,
            clearLocalProgress() {
        ...
        global.debugDumpHiddenLayoutStore = function debugDumpHiddenLayoutStore(floor) {
            try {
                const parsed = getHiddenLayoutStore();
                if (!Number.isFinite(floor)) return clone(parsed);
                return clone(parsed?.floors?.[String(floor)] || null);
            } catch (err) {
