...
class TrueEndingSystem {
    ...
    checkRequirements() {
        // 需要收集所有图腾
        if (!window.totemSystem) return false;
        const totemCount = window.totemSystem.getCount();
        const allTotems = window.totemSystem.getAllTotems().length;
        return totemCount >= allTotems;
    }
    ...
    showUnlockNotification() {
        ...
        notif.innerHTML = `
            ...
            <div style="font-size: 28px; color: #f0f; font-weight: bold; margin-bottom: 15px;">梦境间隙已开启</div>
            <div style="font-size: 16px; color: #ccc; max-width: 400px; line-height: 1.6;">
                你集齐了所有祖先图腾。<br>
                在千根之心，你将有机会与父母并肩作战，<br>
                寻找让他们真正归来的方法。
            </div>
