...
class DebugPanel {
    ...
    createPanel() {
        ...
        panel.id = 'debugPanel';
        panel.style.cssText = 'display:none;position:fixed;top:0;right:0;bottom:0;width:380px;z-index:99999;font-size:12px;color:#fff;pointer-events:none;transition:transform 0.18s ease, opacity 0.18s ease;opacity:1;';
        panel.innerHTML = `
            <button id="debugPanelDock" onclick="window.game.debugPanel.toggleCollapsed()" style="position:absolute;top:18px;left:-34px;width:34px;height:84px;background:rgba(10,14,20,0.92);border:1px solid rgba(135,190,255,0.3);border-right:none;border-radius:10px 0 0 10px;color:#d7efff;cursor:pointer;pointer-events:auto;box-shadow:0 8px 22px rgba(0,0,0,0.35);font-size:16px;font-weight:bold;">◀</button>
            <div id="debugPanelShell" style="height:100%;background:rgba(8,10,14,0.94);border-left:1px solid rgba(135,190,255,0.22);box-shadow:-14px 0 36px rgba(0,0,0,0.38);padding:12px 12px 18px;overflow-y:auto;overflow-x:hidden;pointer-events:auto;backdrop-filter:blur(6px);">
                ...
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;border-bottom:1px solid rgba(135,190,255,0.16);padding-bottom:8px;gap:8px;">
                    ...
                    <div style="display:flex;gap:6px;">
                        ...
                        <button onclick="window.game.debugPanel.toggleCollapsed()" style="background:#255a7a;border:none;color:#fff;border-radius:8px;cursor:pointer;padding:5px 10px;">收起</button>
                ...
                ${section('⚡ 快速控制', `
                    ...
                    <div class="debug-grid-2">
                        ...
                        <button onclick="window.game.debugNextFloor()" style="${buttonBase}background:#ef6c00;">🗺️ 下一层</button>
                    </div>
                ...
                ${section('📊 测试与性能', `
                    ...
                    <div style="margin-top:4px;border-top:1px solid rgba(255,255,255,0.08);padding-top:8px;display:grid;gap:8px;">
                        <div style="display:flex;justify-content:space-between;align-items:center;gap:8px;">
                            <div class="debug-subtitle" style="margin:0;color:#d7efff;">帧耗分析器（父子项会重叠，不要直接相加）</div>
                ...
                ${section('🧩 隐藏房布局编辑', `
                    ...
                    <div class="debug-grid-2">
                        ...
                        <button onclick="window.game.saveHiddenRoomLayout()" style="${buttonBase}background:#2d66a3;">保存布局</button>
                        <button onclick="window.game.resetHiddenRoomLayout()" style="${buttonBase}background:#8c2d2d;">重置布局</button>
    ...
    applyPanelState(forceVisible = false) {
        ...
        const hiddenWidth = 324;
        panel.style.transform = this.collapsed ? `translateX(${hiddenWidth}px)` : 'translateX(0)';
        panel.style.opacity = this.collapsed ? '0.82' : '1';
        if (dock) dock.textContent = this.collapsed ? '▶' : '◀';
        if (dock) dock.title = this.collapsed ? '展开调试边栏' : '收起调试边栏';
        if (shell) shell.style.opacity = this.collapsed ? '0.62' : '1';
    }
    ...
    refreshFrameProfilerStats(force = false) {
        ...
        const summary = [
            ['采样帧', snap.frames || 0],
            ['平均帧', `${Number(snap.avgFrame || 0).toFixed(2)}ms`],
            ['P95', `${Number(snap.p95Frame || 0).toFixed(2)}ms`],
            ['最差帧', `${Number(snap.worstFrame || 0).toFixed(2)}ms`]
        ];
    ...
    refreshRuntimeStats(force = false) {
        ...
        const stats = [
            ['FPS', `${summary.fps || 0} (${summary.avgFps || summary.fps || 0})`],
            ['帧时', `${Number(summary.frameTime || 0).toFixed(1)}ms`],
...
DebugPanel.prototype.refreshHiddenRoomEditorTools = function () {
    ...
    if (status) {
        ...
        status.innerHTML = `当前层 <b>${floor}</b><br>状态 <b>${active ? '编辑中' : '未启用'}</b><br>选中 <b>${selected}</b><br>坐标 <b>${x}, ${y}</b> / 缩放 <b>${scale}</b><br>宽高 <b>${width} x ${height}</b> / 拉伸 <b>${scaleX}, ${scaleY}</b><br>旋转 <b>${rot}°</b> / 层级 <b>${layer}</b><br>最后动作 <b>${lastActionText}</b><br>鼠标 <b>左键拖拽 / 滚轮缩放</b><br>键盘 <b>方向键移动 / [ ] 切换</b>`;
    }
    if (!assetsEl) return;
