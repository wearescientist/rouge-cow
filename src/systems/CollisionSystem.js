...
class CollisionSystem {
    ...
    restrictMovementAgainstShopTable(mover, newX, newY, npc) {
        ...
        let resolveDx = dx;
        let resolveDy = dy;
        if (Math.abs(resolveDx) + Math.abs(resolveDy) < 0.001) {
            resolveDx = mover.x - centerX;
            resolveDy = mover.y - centerY;
        }
        if (Math.abs(resolveDx) + Math.abs(resolveDy) < 0.001) {
            resolveDy = halfHeight;
        }

        const resolveNorm = Math.max(
            0.0001,
            (Math.abs(resolveDx) / Math.max(1, halfWidth)) +
            (Math.abs(resolveDy) / Math.max(1, halfHeight))
        );
        const targetScale = 1 / resolveNorm;
        const safeX = centerX + resolveDx * targetScale;
        const safeY = centerY + resolveDy * targetScale;
        const edgePushX = resolveDx === 0 ? 0 : Math.sign(resolveDx) * 3;
        const edgePushY = resolveDy === 0 ? 0 : Math.sign(resolveDy) * 3;
