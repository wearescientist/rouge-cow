...
(function() {
    ...
    function getSpritePaths(baseId, version, floor, tier) {
        ...
        const layout = `enemies/monster_walk_preserve_features/floor${floor}/T${tier}/${id}`;
        const resolve = (path) => window.RuntimeAssetBase?.resolveSprite?.(path) || `./assets/runtime/sprites/${path}`;
        return {
            ready: resolve(`${layout}/walk/f01.png`),
            walk: resolve(`${layout}/walk/f01.png`),
            attack: resolve(`${layout}/attack/f01.png`),
            hurt: resolve(`${layout}/hurt/f01.png`),
            die: resolve(`${layout}/die/f01.png`)
        };
    }
    ...
    window.ENEMY_VISUAL_CONFIG = VISUAL_CONFIG;
    window.getEnemyVisualConfig = getVisualConfig;
