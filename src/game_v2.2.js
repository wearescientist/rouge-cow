// 肉鸽牛牛 v2.2 - Boss Battles & Weapon Variety
const CONFIG = {
    VERSION: '2.2.0',
    FPS: 60,
    WEAPON_TYPES: ['single', 'dual', 'shotgun', 'spread', 'rapid', 'laser']
};
...
const AudioSynth = {
    ...
    bossSpawn() {
        ...
        setTimeout(() => this.playTone(100, 0.4), 300);
    }
};
        ...
        switch(this.weapon) {
            ...
            case 'shotgun':
                for (let i = -2; i <= 2; i++) {
                    ...
                    bulletsArray.push(new Bullet(this.x, this.y, tx, ty, this.damage * 0.6, isCrit));
                }
                break;
            case 'spread':
                for (let i = 0; i < 8; i++) {
                    ...
                    const tx = this.x + Math.cos(angle) * 300;
                    const ty = this.y + Math.sin(angle) * 300;
                    bulletsArray.push(new Bullet(this.x, this.y, tx, ty, this.damage * 0.5, isCrit));
                }
                break;
            case 'rapid':
                const spread = (Math.random() - 0.5) * 0.3;
                const rAngle = Math.atan2(target.y - this.y, target.x - this.x) + spread;
                const rtx = this.x + Math.cos(rAngle) * 300;
                const rty = this.y + Math.sin(rAngle) * 300;
                bulletsArray.push(new Bullet(this.x, this.y, rtx, rty, this.damage * 0.7, isCrit, 12));
                break;
            case 'laser':
...
class Game {
    ...
    selectUpgrade(index) {
        if (index >= 0 && index < this.pendingUpgrades.length) {
            const upg = this.pendingUpgrades[index];
    ...
    update() {
        ...
        if (!this.boss && this.wave % 5 === 0 && this.waveTimer === 600) {
            this.spawnBoss();
        }
        
        if (!this.boss && this.waveTimer % 1800 === 0) this.wave++;
        
        const target = this.player.update(this.input, this.boss ? [this.boss, ...this.enemies] : this.enemies, this.canvas.width, this.canvas.height);
    ...
    draw() {
        const shakeRestore = this.particles.screenShake(this.ctx, this.canvas);
    ...
    drawUI() {
        ...
        ctx.fillText(`Weapon: ${this.player.weapon}`, 20, y + 30);
        ...
        if (this.showingUpgrade) {
            ...
            for (let i = 0; i < this.pendingUpgrades.length; i++) {
                ...
                const y = this.canvas.height / 2 - 20 + i * 60;
                ctx.fillStyle = '#3498DB';
                ctx.fillRect(this.canvas.width / 2 - 150, y - 25, 300, 50);
                ctx.fillStyle = '#FFF';
                ctx.font = 'bold 20px monospace';
                ctx.fillText(`${i + 1}. ${upg.name}`, this.canvas.width / 2, y);
                ctx.font = '14px monospace';
                ctx.fillStyle = '#CCC';
                ctx.fillText(upg.desc, this.canvas.width / 2, y + 18);
            }
        }
    ...
    gameOver() {
        ...
        this.ctx.fillText(`Survived: ${mins}m ${secs}s | Gold: ${this.gold}`, this.canvas.width / 2, this.canvas.height / 2 + 50);
        this.ctx.fillStyle = '#F1C40F';
        this.ctx.font = '20px monospace';
        this.ctx.fillText('Press SPACE for Menu', this.canvas.width / 2, this.canvas.height / 2 + 100);
...
function showMenu() {
    ...
    menuDiv.innerHTML = `
        <div style="position:fixed;top:0;left:0;width:100%;height:100%;background:#2d1b2e;display:flex;flex-direction:column;align-items:center;color:white;z-index:9999;font-family:monospace;padding:20px;overflow-y:auto;">
            ...
            <p style="font-size:20px;color:#aaa;margin-bottom:20px;">v2.2 - Boss战与武器</p>
            <div style="display:flex;gap:40px;flex-wrap:wrap;justify-content:center;width:100%;max-width:1200px;">
                <div style="background:rgba(255,255,255,0.1);padding:20px;border-radius:10px;min-width:200px;">
                    ...
                    <p>最佳时长: ${Math.floor(save.bestTime / 60)}m ${Math.floor(save.bestTime % 60)}s</p>
                </div>
                ...
                <div style="background:rgba(255,255,255,0.1);padding:20px;border-radius:10px;min-width:250px;">
                    <h3 style="color:#E74C3C;">🔫 武器选择</h3>
                    ${renderWeapon('single', '单发', '基础武器，稳定输出')}
                    ${renderWeapon('dual', '双枪', '两把枪同时射击', save.gold >= 200)}
                    ${renderWeapon('shotgun', '霰弹', '散射伤害，近距离利器', save.gold >= 500)}
                    ${renderWeapon('spread', '环射', '全方位攻击', save.gold >= 800)}
                    ${renderWeapon('rapid', '速射', '极快射速，略低伤害', save.gold >= 1000)}
                    ${renderWeapon('laser', '激光', '高伤害穿透', save.gold >= 1500)}
                </div>
            </div>
    ...
    document.getElementById('startBtn').onclick = () => {
        ...
        currentGame = new Game(selectedWeapon);
        AudioSynth.init();
        AudioSynth.resume();
        currentGame.loop();
    };
...
function renderWeapon(id, name, desc, unlocked = true) {
    const locked = !unlocked;
    return `
        <div class="weapon-btn" data-weapon="${id}" style="padding:10px;margin:5px 0;background:${locked ? '#333' : 'rgba(0,0,0,0.3)'};border-radius:5px;border:2px solid ${id === 'single' ? '#F1C40F' : 'transparent'};cursor:${locked ? 'not-allowed' : 'pointer'};opacity:${locked ? 0.5 : 1};">
            <div style="font-weight:bold;">${locked ? '🔒 ' : ''}${name}</div>
...
console.log('Rougelike Cow v2.2 - Boss System Active');
