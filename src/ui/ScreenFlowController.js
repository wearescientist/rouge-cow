            ...
            <div class="weapon-option" data-weapon="${weapon.key}" style="--w-accent:${weapon.color};">
                <div class="weapon-card-top">
                    <div class="weapon-icon-badge">${window.WeaponIconResolver
                        ...
                        : (weapon.iconSprite ? window.AssetResolver ? window.AssetResolver.createWeaponIconMarkup(weapon.iconSprite, weapon.icon, weapon.name, 'width:72%;height:72%;object-fit:contain;filter:drop-shadow(0 4px 10px rgba(0,0,0,0.35));') : `<img src="${window.RuntimeAssetBase?.resolveSprite?.(`weapons/${weapon.iconSprite}.png`) || `assets/runtime/sprites/weapons/${weapon.iconSprite}.png`}" alt="${weapon.name}" style="width:72%;height:72%;object-fit:contain;filter:drop-shadow(0 4px 10px rgba(0,0,0,0.35));">` : weapon.icon)}</div>
