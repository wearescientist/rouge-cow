class SeamlessPrologueController {
    ...
    createScenes() {
        const configuredScenes = window.StoryDialogueData?.prologueScenes;
        if (Array.isArray(configuredScenes) && configuredScenes.length > 0) {
            return configuredScenes.map(scene => ({
                ...scene,
                image: scene.image ? this.resolveSceneImage(scene.image) : scene.image,
                bubbles: Array.isArray(scene.bubbles) ? scene.bubbles.map(bubble => ({ ...bubble })) : []
            }));
        }
        ...
        return [
            ...
            {
                key: 'empty_home',
                image: this.resolveSceneImage('story/start2.png'),
                duration: 11800,
            ...
            {
                key: 'note',
                image: this.resolveSceneImage('story/start3.png'),
                duration: 12600,
            ...
            {
                key: 'descent',
                image: this.resolveSceneImage('story/start4.png'),
                duration: 13200,
                ...
                bridgeText: '先拿一个像样的家伙，再下去找你们。'
            }
        ];
    }

    resolveSceneImage(path = '') {
        return window.RuntimeAssetBase?.resolveSprite?.(String(path || '').replace(/^assets\/runtime\/sprites\//, '').replace(/^story\//, 'story/'))
            || window.RuntimeAssetBase?.resolve?.(path)
            || path;
    }
    ...
    init() {
        ...
        this.root.innerHTML = `
            ...
            <div class="seamless-prologue__skip">空格/点击继续，Esc 跳过</div>
        `;
