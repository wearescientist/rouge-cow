class OverlayCanvasRenderer {
    ...
    isCompactViewport() {
        return !!(this.game?.isMobileDevice || (typeof window !== 'undefined' && (window.innerWidth <= 900 || window.innerHeight <= 620)));
    }
    ...
    fitPanel(metrics, designW, designH, margin = 26) {
        ...
        const safeMargin = Math.max(compact ? 18 : 14, margin * metrics.scale * (compact ? 1.16 : 1));
        const mobileCap = compact ? (typeof window !== 'undefined' && window.innerWidth > window.innerHeight ? 0.86 : 0.9) : 1;
