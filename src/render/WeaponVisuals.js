...
class WeaponVisuals {
    ...
    getBulletVisualConfigKey(bullet) {
        const weaponKey = bullet?.weaponKey || null;
        if (!weaponKey) return null;
        if (bullet?.renderStyle === 'holy_sword_wave') {
            if (weaponKey === 'blood_whip') return 'blood_whip_wave';
            if (weaponKey === 'whip') return 'whip_wave';
        }
        return weaponKey;
    }

    getWeaponRenderMode(bullet) {
        return this.getWeaponVisualConfig(this.getBulletVisualConfigKey(bullet))?.renderMode || 'auto';
    }

    shouldRenderAfterForegroundOcclusion(bullet) {
        if (!bullet) return false;
        if (bullet.renderStyle === 'holy_sword_wave') return true;
        return bullet.weaponKey === 'whip' || bullet.weaponKey === 'blood_whip';
    }

    getPlayerVisualRadius(player) {
        if (!player) return 20;

        if (typeof player.getCollisionRadius === 'function') {
            const collisionRadius = player.getCollisionRadius();
            if (Number.isFinite(collisionRadius) && collisionRadius > 0) {
                return collisionRadius;
            }
        }

        const spriteData = player.spriteData;
        const scale = typeof player.getCollisionScale === 'function'
            ? player.getCollisionScale()
            : 1;
        if (spriteData?.bounds) {
            const modelWidth = (spriteData.bounds.width || 0) * scale;
            const modelHeight = (spriteData.bounds.height || 0) * scale;
            const hullRadius = Math.max(modelWidth, modelHeight) * 0.5;
            if (Number.isFinite(hullRadius) && hullRadius > 0) {
                return Math.max(12, hullRadius);
            }
        }

        const fallbackSize = Number.isFinite(player.size) ? player.size : 40;
        return Math.max(12, fallbackSize * 0.5);
    }

    drawOrbitLockedLineSprite(ctx, sprite, bounds, bullet, visualCfg, options = {}) {
        if (!ctx || !sprite || !bounds) return false;

        const sourceWidth = Math.max(1, bounds.width);
        const sourceHeight = Math.max(1, bounds.height);
        const gripPoint = visualCfg.gripPoint || { x: 0.9, y: 0.1 };
        const tipPoint = visualCfg.tipPoint || { x: 0.1, y: 0.9 };
        const gripLocalX = sourceWidth * gripPoint.x;
        const gripLocalY = sourceHeight * gripPoint.y;
        const tipLocalX = sourceWidth * tipPoint.x;
        const tipLocalY = sourceHeight * tipPoint.y;
        const localDx = tipLocalX - gripLocalX;
        const localDy = tipLocalY - gripLocalY;
        const localLength = Math.hypot(localDx, localDy);
        if (localLength < 1e-3) return false;

        const playerRadius = Number.isFinite(options.playerRadius)
            ? options.playerRadius
            : this.getPlayerVisualRadius(options.player);
        const gripGap = Number.isFinite(visualCfg.gripGap) ? visualCfg.gripGap : 4;
        const attackRadius = Number.isFinite(options.attackRadius)
            ? options.attackRadius
            : (Number.isFinite(bullet.attackRadius) ? bullet.attackRadius : bullet.radius);
        const orbitRadius = Number.isFinite(options.orbitRadius)
            ? options.orbitRadius
            : (Number.isFinite(bullet.radius) ? bullet.radius : attackRadius);
        if (!Number.isFinite(attackRadius) || !Number.isFinite(orbitRadius)) return false;

        const gripDistance = Math.max(0, playerRadius + gripGap);
        const tipDistance = Math.max(gripDistance + 12, attackRadius);
        const targetLength = Math.max(12, tipDistance - gripDistance);
        const scale = targetLength / localLength;
        const targetAngle = options.baseAngle || 0;
        const localAngle = Math.atan2(localDy, localDx);
        const rotation = targetAngle - localAngle;
        const cos = Math.cos(rotation);
        const sin = Math.sin(rotation);
        const gripTargetDistance = gripDistance - orbitRadius;
        const gripTargetX = Math.cos(targetAngle) * gripTargetDistance;
        const gripTargetY = Math.sin(targetAngle) * gripTargetDistance;
        const tx = gripTargetX - scale * (cos * gripLocalX - sin * gripLocalY);
        const ty = gripTargetY - scale * (sin * gripLocalX + cos * gripLocalY);

        ctx.save();
        ctx.transform(scale * cos, scale * sin, -scale * sin, scale * cos, tx, ty);
        ctx.drawImage(
            sprite,
            bounds.x,
            bounds.y,
    ...
    getOrbitSpriteTransformData(bullet, options = {}) {
        const visualCfg = this.getWeaponVisualConfig(this.getBulletVisualConfigKey(bullet)) || {};
        const spriteKey = visualCfg.spriteKey || bullet?.weaponSprite || this.weaponSpriteMap[bullet?.weaponKey || ''] || null;
        const sprite = spriteKey ? this.game.sprites?.get(spriteKey) : null;
        ...
        const runtimeSize = Number.isFinite(options.fallbackSize)
            ...
            : (Number.isFinite(bullet?.size) ? bullet.size : null);
        ...
        const targetSize = Math.max(options.minSize || 8, baseSize * zoom * scale);
        const scaleFactor = targetSize / Math.max(1, bounds.height);
    ...
    rotateLocalPoint(point, rotation) {
        const cos = Math.cos(rotation);
        const sin = Math.sin(rotation);
        return {
    ...
    getHolySwordHitProfile(bullet, options = {}) {
        const transform = this.getOrbitSpriteTransformData(bullet, {
            ...options,
            scene: 'orbit'
        });
        if (!transform) return null;

        const cfg = transform.visualCfg || {};
        const gripPoint = cfg.hitGripPoint || cfg.gripPoint || { x: 0.82, y: 0.18 };
        const tipPoint = cfg.hitTipPoint || cfg.tipPoint || { x: 0.10, y: 0.92 };
        const localGrip = {
            x: transform.offsetX - transform.drawWidth * transform.anchorX + transform.drawWidth * gripPoint.x,
            y: transform.offsetY - transform.drawHeight * transform.anchorY + transform.drawHeight * gripPoint.y
        };
        const localTip = {
            x: transform.offsetX - transform.drawWidth * transform.anchorX + transform.drawWidth * tipPoint.x,
            y: transform.offsetY - transform.drawHeight * transform.anchorY + transform.drawHeight * tipPoint.y
        };
        const rotatedGrip = this.rotateLocalPoint(localGrip, transform.rotation);
        const rotatedTip = this.rotateLocalPoint(localTip, transform.rotation);
        const player = options.player || this.game?.player || null;
        const playerX = Number.isFinite(player?.cx) ? player.cx : 0;
        const playerY = Number.isFinite(player?.cy) ? player.cy : 0;
        const radialX = (bullet.x || 0) - playerX;
        const radialY = (bullet.y || 0) - playerY;
        const radialLen = Math.hypot(radialX, radialY);
        const dirX = radialLen > 1e-6 ? (radialX / radialLen) : Math.cos(transform.rotation);
        const dirY = radialLen > 1e-6 ? (radialY / radialLen) : Math.sin(transform.rotation);
        const playerRadius = Number.isFinite(options.playerRadius)
            ? options.playerRadius
            : this.getPlayerVisualRadius(player);
        const attackRadius = Number.isFinite(options.attackRadius)
            ? options.attackRadius
            : (Number.isFinite(bullet.attackRadius) ? bullet.attackRadius : (Number.isFinite(bullet.radius) ? bullet.radius : radialLen));
        const innerGap = Number.isFinite(cfg.hitInnerGap) ? cfg.hitInnerGap : 6;
        const outerExtend = Number.isFinite(cfg.hitOuterExtend) ? cfg.hitOuterExtend : 8;
        const desiredStartDistance = Math.max(0, playerRadius + innerGap);
        const desiredEndDistance = Math.max(desiredStartDistance + 12, attackRadius + outerExtend);
        const actualGripX = bullet.x + rotatedGrip.x;
        const actualGripY = bullet.y + rotatedGrip.y;
        const actualTipX = bullet.x + rotatedTip.x;
        const actualTipY = bullet.y + rotatedTip.y;
        const gripProjected = (actualGripX - playerX) * dirX + (actualGripY - playerY) * dirY;
        const tipProjected = (actualTipX - playerX) * dirX + (actualTipY - playerY) * dirY;
        const actualStartDistance = Math.min(gripProjected, tipProjected);
        const actualEndDistance = Math.max(gripProjected, tipProjected);
        const startDistance = Math.min(actualStartDistance, desiredStartDistance);
        const endDistance = Math.max(actualEndDistance, desiredEndDistance);
        return {
            ax: playerX + dirX * startDistance,
            ay: playerY + dirY * startDistance,
            bx: playerX + dirX * endDistance,
            by: playerY + dirY * endDistance,
            halfWidth: Math.max(7, Number.isFinite(cfg.hitWidth) ? cfg.hitWidth * 0.5 : transform.targetSize * 0.12)
        };
    }

    holySwordOrbHitsEnemy(bullet, enemy, options = {}) {
        if (!bullet || !enemy) return false;
        const profile = this.getHolySwordHitProfile(bullet, options);
        if (!profile) return false;

        const enemyX = Number.isFinite(enemy.cx) ? enemy.cx : (enemy.x || 0);
        let enemyY = Number.isFinite(enemy.cy) ? enemy.cy : 0;
        if (typeof enemy.getCenterY === 'function') {
            const centerY = enemy.getCenterY();
            if (Number.isFinite(centerY)) enemyY = centerY;
        } else if (!Number.isFinite(enemyY)) {
            enemyY = enemy.y || 0;
        }
        const enemyRadius = typeof enemy.getCollisionRadius === 'function'
            ? (enemy.getCollisionRadius() || Math.max(8, (enemy.size || 24) * 0.4))
            : Math.max(8, (enemy.size || 24) * 0.4);

        return this.distancePointToSegment(enemyX, enemyY, profile.ax, profile.ay, profile.bx, profile.by)
            <= (enemyRadius + profile.halfWidth);
    }
    ...
    drawConfiguredProjectileSprite(ctx, bullet, zoom, options = {}) {
        ...
        const visualCfg = transform?.visualCfg || {};
        const spriteKey = visualCfg.spriteKey || bullet.weaponSprite || this.weaponSpriteMap[bullet.weaponKey || ''] || null;
        const sprite = spriteKey ? this.game.sprites?.get(spriteKey) : null;
        if (!ctx || !sprite || !transform) return false;
        const bounds = transform.bounds;

        if ((options.scene || visualCfg.scene || 'projectile') === 'orbit' && visualCfg.orbitLockLine) {
            return this.drawOrbitLockedLineSprite(ctx, sprite, bounds, bullet, visualCfg, options);
        }

        const targetSize = transform.targetSize;
    ...
    drawTrimmedSprite(ctx, sprite, options = {}) {
        ...
        const bounds = options.bounds || this.getSpriteContentBounds(sprite);
    ...
    drawProjectile(bullet, screenX, screenY, zoom, time) {
        ...
        } else if (style === 'holy_sword_wave') {
            this.drawHolySwordWaveProjectile(ctx, bullet, width, angle, time, zoom);
        } else {
    ...
    drawHolyWaterBottleThrow(bullet, screenX, screenY, zoom, time) {
        ...
        if (age <= flight) {
            ...
            this.ctx.rotate(rot);
            const s = Math.max(14, (bullet.bottleSize || 18) * zoom);
            this.ctx.shadowColor = 'rgba(140, 220, 255, 0.55)';
            this.ctx.shadowBlur = 10 * zoom;
            if (bottle) this.ctx.drawImage(bottle, -s / 2, -s / 2, s, s);
            else {
                this.ctx.fillStyle = 'rgba(140,220,255,0.9)';
                this.ctx.fillRect(-4 * zoom, -7 * zoom, 8 * zoom, 14 * zoom);
            }
            this.ctx.restore();
        } else {
    ...
    drawCrossProjectile(ctx, bullet, width, time, zoom) {
        ...
        const spin = (typeof bullet.spin === 'number')
            ? bullet.spin
            : time * (((visualCfg.spinSpeedDeg || 344) * Math.PI) / 180) * (bullet.weaponKey === 'heaven_sword' ? 1.12 : 0.3);
        const renderMode = this.getWeaponRenderMode(bullet);
        if (renderMode === 'sprite' && this.drawConfiguredProjectileSprite(ctx, bullet, zoom, {
            spin,
            minSize: Math.max(8, width)
    ...
    drawHolySwordWaveProjectile(ctx, bullet, width, angle, time, zoom) {
        const renderMode = this.getWeaponRenderMode(bullet);
        const spinSpeedDeg = Number.isFinite(bullet.spinSpeedDeg)
            ? bullet.spinSpeedDeg
            : (this.getWeaponVisualConfig(this.getBulletVisualConfigKey(bullet))?.spinSpeedDeg || 500);
        const spin = (typeof bullet.spin === 'number')
            ? bullet.spin
            : time * ((spinSpeedDeg * Math.PI) / 180) + (bullet.visualSeed || 0) * 0.003;
        if (renderMode === 'sprite' || renderMode === 'both') {
            if (this.drawConfiguredProjectileSprite(ctx, bullet, zoom, {
                scene: 'projectile',
                baseAngle: angle || 0,
                spin,
                minSize: Math.max(12, width)
            })) return;
        }

        const bladeLen = Math.max(24, width * 1.7);
        const bladeWidth = Math.max(10, width * 0.62);
        ctx.save();
        ctx.rotate((angle || 0) + spin);
        this.drawFallbackProjectile(ctx, bullet, bladeLen, bladeWidth);
        ctx.restore();
    }
    ...
    getPresentationEmissiveSpec(entity) {
        ...
        if (key === 'whip' || key === 'blood_whip') return this.presentationEmissiveConfig.cross;
