...
class HD2DRenderer {
    constructor(canvas, ctx) {
        ...
        this.calmPostProcessFrame = 0;
        this.enablePlayerVignette = true;
        this.activeGraphicsQuality = 'high';
    }
    ...
    renderFinalPostProcess(playerScreenX, playerScreenY) {
        ...
        const roomBlurMode = this.getRoomBlurMode();
        const useCalmColorGrading = this.shouldUseCalmColorGrading();
        const game = window.game || null;
        const room = game?.curRoom || null;
        const runtimeSettings = game?.runtimeSettings || {};
        ...
        if (roomBlurMode !== 'off') {
            this.roomBlur.render(this.postProcessFocus.x, this.postProcessFocus.y, { mode: roomBlurMode });
        }

        if (roomBlurMode === 'off' && this.enablePlayerVignette !== false && allowCurrentRoomVignette) {
            this.renderPlayerVignette(focusX, focusY);
        }
        if (!useCalmColorGrading) {
            this.colorGrading.render();
        }
    }

    getRoomBlurMode() {
        if (!(this.roomBlur && this.roomBlur.enabled)) return 'off';
        const game = window.game || null;
        const room = game?.curRoom || null;
        if (room?.type === 'hidden') return 'heavy';
        if (Number(room?.floor) === 7 && room?.floor7Role === 'awakening') return 'heavy';
        if (game?.forceHeavyRoomBlur === true) return 'heavy';
        if (room?.type === 'shop' || room?.type === 'treasure') return 'calm';
        return 'off';
    }

    shouldUseCalmColorGrading() {
        const game = window.game || null;
        const room = game?.curRoom || null;
        if (Number(room?.floor) === 7 && room?.floor7Role === 'awakening') return false;
        return room?.type !== 'hidden';
    }
    ...
    renderPlayerVignette(x, y) {
        ...
        const allowHiddenRoomVignette = vignetteEnabled && runtimeSettings.enableHiddenRoomVignette !== false;
        const allowAwakeningRoomVignette = vignetteEnabled && runtimeSettings.enableAwakeningRoomVignette !== false;
        const allowCurrentRoomVignette = isAwakeningRoom ? allowAwakeningRoomVignette : (hiddenRoom ? allowHiddenRoomVignette : allowOtherRoomVignette);
