...
class RoomBlurSystem {
    constructor(ctx) {
        ...
        this.enableBloomLayer = true;
        this.enableColorLayer = true;
        this.enablePresentationMask = true;
        this.enableDarknessLayer = true;
        this.enableRoomFeatureLights = true;
        this.enableDynamicPlayerLight = true;
        ...
        this.maxHiddenLights = 32;
        this.maxHiddenBloomLights = 20;
        this.maxHiddenColorLights = 18;
        ...
        this.currentRenderMode = 'heavy';
        this.calmCompositeThrottleMs = 140;
        this.calmLightCacheMs = 220;
        this.calmFocusThreshold = 28;
        this.calmCache = {
            roomKey: '',
            mode: '',
            lastCompositeAt: 0,
    ...
    isPinnedHiddenLight(light) {
        ...
        if (light.pinHidden === true) return true;
        const kind = String(light?.kind || '');
        return /^(hidden_template_mushroom|hidden_mushroom|hidden_memory_mushroom|hidden_placed_decor_mushroom|hidden_decor_mushroom|hidden_candle|hidden_seal_target|hidden_seal_blocker|hidden_seal_room|hidden_orb|legacy_|floor2_worm_red|rabbit_demo)/.test(kind);
    }
    ...
    getLightPriority(light, focusX, focusY, hiddenRoom = false) {
        ...
        if (/player_presence|player_focus|player_warmth/.test(kind)) priority += 5.0;
        ...
        if (/shop_candle|shopkeeper/.test(kind)) priority += 2.0;
        if (/treasure_chest/.test(kind)) priority += 1.6;
    ...
    trimLightSourcesForPass(lightSources, hiddenRoom, pass = 'bloom') {
        ...
        if (!hiddenRoom) return lightSources.slice(0, scaled);
        const pinned = lightSources.filter((light) => this.isPinnedHiddenLight(light));
        const rest = lightSources.filter((light) => !this.isPinnedHiddenLight(light));
        const keepCount = Math.max(scaled, pinned.length);
        return [...pinned, ...rest].slice(0, keepCount);
    }


    getAdaptiveProfile(game, width, height) {
        if (!this.adaptiveQuality) {
            return { tier: 'full', bloomEvery: 1, colorEvery: 1, lightBudgetScale: 1, bloomAlphaScale: 1, colorAlphaScale: 1, compositeThrottleMs: 0, lightCacheMs: 0, focusThreshold: 0 };
        }
        const room = game?.curRoom || null;
        const hiddenRoom = room?.type === 'hidden' || (Number(room?.floor) === 7 && room?.floor7Role === 'awakening');
        if (hiddenRoom) {
            return { tier: 'full', bloomEvery: 1, colorEvery: 1, lightBudgetScale: 1.15, bloomAlphaScale: 1, colorAlphaScale: 1, compositeThrottleMs: 0, lightCacheMs: 0, focusThreshold: 0 };
        }
        const budgetManager = (typeof window !== 'undefined') ? window.RenderBudgetManager : null;
        if (budgetManager && typeof budgetManager.getBudget === 'function') {
            const shared = budgetManager.getBudget(game, room, width, height);
            if (shared?.blurProfile) {
                return {
                    tier: shared.blurProfile.tier || 'full',
                    bloomEvery: shared.blurProfile.bloomEvery || 1,
                    colorEvery: shared.blurProfile.colorEvery || 1,
                    lightBudgetScale: shared.blurProfile.lightBudgetScale ?? 1,
                    bloomAlphaScale: shared.blurProfile.bloomAlphaScale ?? 1,
                    colorAlphaScale: shared.blurProfile.colorAlphaScale ?? 1,
                    compositeThrottleMs: shared.blurProfile.compositeThrottleMs || 0,
                    lightCacheMs: shared.blurProfile.lightCacheMs || 0,
                    focusThreshold: shared.blurProfile.focusThreshold || 0
                };
            }
        }
        ...
        if (pressure >= 4.4) {
            return { tier: 'low', bloomEvery: 3, colorEvery: 2, lightBudgetScale: 0.55, bloomAlphaScale: 0.72, colorAlphaScale: 0.76, compositeThrottleMs: 0, lightCacheMs: 0, focusThreshold: 0 };
        }
        if (pressure >= 3.0) {
            return { tier: 'medium', bloomEvery: 2, colorEvery: 1, lightBudgetScale: 0.72, bloomAlphaScale: 0.84, colorAlphaScale: 0.88, compositeThrottleMs: 0, lightCacheMs: 0, focusThreshold: 0 };
        }
        return { tier: 'full', bloomEvery: 1, colorEvery: 1, lightBudgetScale: 1, bloomAlphaScale: 1, colorAlphaScale: 1, compositeThrottleMs: 0, lightCacheMs: 0, focusThreshold: 0 };
    }

    getRenderRoomKey(room, width, height) {
        return room?.id || `${room?.floor || 0}:${room?.type || 'room'}:${room?.floor7Role || ''}:${width}x${height}`;
    }

    shouldRefreshCalmLightSources(roomKey, focusX, focusY, timeMs, adaptive) {
        const cache = this.calmCache;
        if (!cache.lightSources || cache.roomKey !== roomKey || cache.mode !== 'calm') return true;
        if ((timeMs - (cache.lastLightAt || 0)) >= Math.max(80, adaptive.lightCacheMs || this.calmLightCacheMs)) return true;
        const dx = (focusX || 0) - (cache.lastFocusX || 0);
        const dy = (focusY || 0) - (cache.lastFocusY || 0);
        const threshold = Math.max(12, adaptive.focusThreshold || this.calmFocusThreshold);
        return (dx * dx + dy * dy) >= (threshold * threshold);
    }

    shouldRefreshCalmComposite(roomKey, timeMs, adaptive, refreshedLights) {
        const cache = this.calmCache;
        if (refreshedLights) return true;
        if (cache.roomKey !== roomKey || cache.mode !== 'calm') return true;
        return (timeMs - (cache.lastCompositeAt || 0)) >= Math.max(80, adaptive.compositeThrottleMs || this.calmCompositeThrottleMs);
    }
    ...
    normalizeLightSource(light, camera, index, time) {
        ...
        const isPlayerFocus = kind === 'player_focus' || kind === 'player_presence' || kind === 'player_warmth' || kind === 'hidden_focus_candle' || kind === 'hidden_focus_mushroom';
    ...
    collectTreasureSources(game) {
        const room = game?.curRoom;
        if (!room || room.type !== 'treasure') return [];
        const list = [];
        const pushChest = (chest, index = 0) => {
            if (!chest || chest.opened || chest.disabled) return;
            ...
            list.push({
                ...
                alpha: isLegendary ? 0.72 : (isRare ? 0.60 : 0.50),
                reveal: isLegendary ? 1.06 : (isRare ? 0.92 : 0.84),
                clarity: isLegendary ? 1.14 : (isRare ? 1.06 : 1.00),
                pulseSpeed: 2.2 + index * 0.18,
                pulseAmount: isLegendary ? 0.09 : 0.07,
                bloomAlpha: isLegendary ? 0.32 : (isRare ? 0.25 : 0.19),
                haloAlpha: isLegendary ? 0.14 : (isRare ? 0.11 : 0.09),
                haloRadiusMul: isLegendary ? 1.38 : (isRare ? 1.28 : 1.20),
                colorizeAlpha: isLegendary ? 0.34 : (isRare ? 0.28 : 0.18),
                colorizeRadiusMul: isLegendary ? 1.24 : (isRare ? 1.18 : 1.12),
                preserveSharpness: isLegendary ? 1.06 : (isRare ? 0.96 : 0.88),
                preferColor: true
            });
        };
        if (Array.isArray(room.chests) && room.chests.length > 0) {
            room.chests.forEach((chest, index) => pushChest(chest, index));
        } else if (room.chest) {
            pushChest(room.chest, 0);
        }
        return list;
    }
    ...
    collectFocusSources(focusX, focusY, canvasScale) {
        ...
        const sources = [];
        if (hiddenRoom) {
            if (this.enableDarknessLayer !== false) {
                sources.push({
            ...
            if (this.enableDynamicPlayerLight !== false) {
                sources.push({
                    kind: 'player_warmth',
                    screenX: centerX,
                    screenY: centerY + 6 * canvasScale,
                    radiusX: 108 * canvasScale,
                    radiusY: 84 * canvasScale,
                    bloomRadius: 54 * canvasScale,
                    color: { r: 255, g: 214, b: 166 },
                    alpha: 0.14,
                    reveal: 0.16,
                    clarity: 0.16,
                    bloomAlpha: 0.12,
        ...
        if (this.enableDynamicPlayerLight !== false) {
            sources.push({
                kind: 'player_warmth',
                screenX: centerX,
                screenY: centerY + 8 * canvasScale,
                radiusX: 132 * canvasScale,
                radiusY: 104 * canvasScale,
                bloomRadius: 64 * canvasScale,
                color: { r: 255, g: 218, b: 178 },
                alpha: 0.18,
                reveal: 0.18,
                clarity: 0.18,
                bloomAlpha: 0.15,
    ...
    collectUnifiedLightSources(game, focusX, focusY, canvasScale, time) {
        const includeRoomFeatures = this.enableRoomFeatureLights !== false;
        const raw = [
            ...this.collectFocusSources(focusX, focusY, canvasScale),
            ...(includeRoomFeatures ? this.collectRoomSources(game, time) : [])
        ];
        ...
        const room = game?.curRoom || null;
        const hiddenRoom = room?.type === 'hidden' || (Number(room?.floor) === 7 && room?.floor7Role === 'awakening');
        return this.prioritizeLightSources(sources, focusX, focusY, hiddenRoom);
    }

    renderPresentationMask(game, time, width, height) {
        const ctx = this.presentationMaskCtx;
        ctx.clearRect(0, 0, width, height);
        if (this.enablePresentationMask === false) return;
        const room = game?.curRoom;
    ...
    renderBlurLayer(lightSources, width, height) {
        ...
        lightSources.forEach((light) => {
            ...
            const playerLikeFocus = light.kind === 'player_focus' || light.kind === 'player_presence' || light.kind === 'player_warmth' || light.kind === 'hidden_focus_candle' || light.kind === 'hidden_focus_mushroom';
    ...
    renderDarknessLayer(lightSources, width, height) {
        ...
        ctx.clearRect(0, 0, width, height);
        if (this.enableDarknessLayer === false) return;
        const room = window.game?.curRoom || null;
        const runtimeSettings = window.game?.runtimeSettings || {};
        const isHiddenRoom = room?.type === 'hidden';
        const isAwakeningRoom = Number(room?.floor) === 7 && room?.floor7Role === 'awakening';
        const hiddenRoom = isHiddenRoom || isAwakeningRoom;
        const vignetteEnabled = runtimeSettings.enableVignette !== false;
        const allowOtherRoomVignette = vignetteEnabled && runtimeSettings.enableOtherRoomVignette !== false;
        const allowHiddenRoomVignette = vignetteEnabled && runtimeSettings.enableHiddenRoomVignette !== false;
        const allowAwakeningRoomVignette = vignetteEnabled && runtimeSettings.enableAwakeningRoomVignette !== false;
        const allowCurrentRoomVignette = isAwakeningRoom ? allowAwakeningRoomVignette : (isHiddenRoom ? allowHiddenRoomVignette : allowOtherRoomVignette);
        if (!allowCurrentRoomVignette) return;
        const floor7DarkAlpha = Number.isFinite(room?.floor7AwakeningDarknessAlpha) ? room.floor7AwakeningDarknessAlpha : null;
        const hiddenDarkAlpha = floor7DarkAlpha != null ? floor7DarkAlpha : 0.82;
        const calmMode = this.currentRenderMode === 'calm';
        const outerDimAlpha = calmMode ? Math.min(0.18, this.outerDimStrength * 0.68) : this.outerDimStrength;
        ctx.fillStyle = `rgba(2, 3, 5, ${hiddenRoom ? hiddenDarkAlpha : outerDimAlpha})`;
        ...
        lightSources.forEach((light) => {
            if (light.selfVisibleOnly) return;
            const hiddenRevealMul = /^(hidden_|legacy_|floor2_worm|hidden_orb|room_|worm_|ambient_worm|secret_worm|rabbit_|player_presence|player_warmth)/.test(light.kind) ? 0.96 : 0.82;
        ...
        if (this.enablePresentationMask !== false) {
    ...
    renderBloomLayer(lightSources, width, height) {
        ...
        ctx.clearRect(0, 0, width, height);
        if (this.enableBloomLayer === false) return;
        const room = window.game?.curRoom || null;
        const hiddenRoom = room?.type === 'hidden' || (Number(room?.floor) === 7 && room?.floor7Role === 'awakening');
    ...
    renderColorLayer(lightSources, width, height) {
        ...
        ctx.clearRect(0, 0, width, height);
        if (this.enableColorLayer === false) return;
        const room = window.game?.curRoom || null;
        const hiddenRoom = room?.type === 'hidden' || (Number(room?.floor) === 7 && room?.floor7Role === 'awakening');
    ...
    render(focusX, focusY, options = {}) {
        ...
        const room = game?.curRoom || null;
        const hiddenRoom = room?.type === 'hidden' || (Number(room?.floor) === 7 && room?.floor7Role === 'awakening');
        const mode = options?.mode || (hiddenRoom ? 'heavy' : 'off');
        const calmMode = mode === 'calm';
        const roomKey = this.getRenderRoomKey(room, w, h);
        ...
        const refreshLightSources = calmMode
            ? this.shouldRefreshCalmLightSources(roomKey, focusX, focusY, timeMs, adaptive)
            : true;
        const lightSources = refreshLightSources
            ? this.collectUnifiedLightSources(game, focusX, focusY, canvasScale, time)
            : (this.calmCache.lightSources || []);
        if (calmMode && refreshLightSources) {
            this.calmCache.lightSources = lightSources;
            this.calmCache.lastLightAt = timeMs;
            this.calmCache.lastFocusX = focusX;
            this.calmCache.lastFocusY = focusY;
        }
        const refreshComposite = calmMode
            ? this.shouldRefreshCalmComposite(roomKey, timeMs, adaptive, refreshLightSources)
            : true;

        if (refreshComposite) {
            this.renderPresentationMask(game, time, w, h);
            if (!hiddenRoom && this.useFullScreenBlur && !calmMode) {
            ...
            if (calmMode) {
                this.calmCache.roomKey = roomKey;
                this.calmCache.mode = 'calm';
                this.calmCache.lastCompositeAt = timeMs;
                this.calmCache.lastFocusX = focusX;
                this.calmCache.lastFocusY = focusY;
            }
        }
        ...
        this.ctx.globalAlpha = hiddenRoom ? 0.66 : (calmMode ? 0.82 : 1);
