class Enemy {
    ...
    getCollisionRadius() {
        // 尝试从 SpriteData 获取
        if (window.game && window.game.spriteDataRegistry) {
            const spriteData = window.game.spriteDataRegistry.get(this.type);
            if (spriteData) {
                // 使用模型宽度的一半作为碰撞半径（考虑 hitboxRatio）
                return (spriteData.modelWidth * spriteData.hitboxRatio.w) / 2;
            }
        }
        // 回退到旧逻辑
    ...
    getCenterY() {
        if (this.spriteData && this.spriteData.getCenterPosition) {
            const center = this.spriteData.getCenterPosition(this.x, this.y, 1, 'feet');
            return center.y;
        }
        // 回退：基于 size 的估算
    ...
    takeDamage(dmg, stats = {}) {
        ...
        if (window.game && window.game.audio) {
            ...
            const ENEMY_MATERIALS = {
                // T1 小怪
                'chick': 'bird', 'mouse': 'fur', 'snail': 'shell', 'pigeon': 'bird',
    ...
    getTargetHeight(sprites) {
        ...
        // 获取贴图实际高度（优先用描边贴图，其次基础贴图）
        let baseHeight = 32; // 默认基础高度
        if (sprites) {
            const outlinedName = this.getOutlinedSpriteName();
            const sprite = sprites.get(outlinedName) || sprites.get(this.sprite);
            if (sprite) {
                baseHeight = sprite.height || 32;
            }
        }
        ...
        const multiplier = tierMultipliers[this.tier] || 1.0;
    ...
    drawWithOffset(ctx, sprites, floor) {
        ...
        ctx.font = '10px Arial';
        ctx.textAlign = 'center';
        const aspectRatio = (spriteW / spriteH).toFixed(2);
        ctx.fillText(`${spriteW}x${spriteH} R:${aspectRatio}`, 0, barY + barHeight + 12);
