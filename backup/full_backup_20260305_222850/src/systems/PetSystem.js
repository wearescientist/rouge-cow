...
class Pet {
    ...
    render(ctx, camera, sprites) {
        ...
        // v0.30-iter3: 优化宠物绘制参数
        let drawX = -13, drawY = -13, drawW = 26, drawH = 26;
        const PET_TARGET_SIZE = 26; // 宠物目标显示尺寸
        ...
        if (sprite && window.spriteDataRegistry) {
            ...
            if (spriteData) {
                ...
                if (modelRatio > 1) {
                    drawW = PET_TARGET_SIZE;
                    drawH = PET_TARGET_SIZE / modelRatio;
                } else {
                    drawH = PET_TARGET_SIZE;
                    drawW = PET_TARGET_SIZE * modelRatio;
                }
                
                // 根据锚点计算偏移
                const anchor = spriteData.anchor.feet;
                drawX = -drawW * (anchor.x / spriteData.canvasWidth);
                drawY = -drawH * (anchor.y / spriteData.canvasHeight);
            }
        }
